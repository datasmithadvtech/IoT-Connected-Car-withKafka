(function ( $ ) {
    ...
    var methods = {
        init : function(options) {
            ...
            var degre = $("<div class='progress-degre'></div>");
             if(!global_settings.degree) {
                percentage.text(global_settings.degree);
            }
            ...
            else if(global_settings.degree){
              $(this).append(degre); 
                mod = 255;
            ...
            if(global_settings.starting_position != 100) {
                global_settings.starting_position = global_settings.starting_position % mod;
            }
            if(global_settings.ending_position != 100) {
                global_settings.ending_position = global_settings.ending_position % mod;
            }
            // No 'px' or '%', add 'px'
    ...
    function calcPos(apples, percent) {
        if(percent < 0) {
            ...
            var starting_degree = (parseInt(apples) / mod) * 360;
            var starting_radian = starting_degree * (Math.PI / 180);
            return starting_radian - (Math.PI / 2);
        } else {
            // Calculate ending position
            var ending_degree = ((parseInt(apples) + parseInt(percent)) / mod) * 360;
	...
	function insertText2(scope) {
        $(".progress-degre").text(Math.round(global_settings.percent) + "°");
    }
    // create canvas
